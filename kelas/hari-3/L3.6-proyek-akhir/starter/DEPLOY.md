# Deployment on-premise dengan Docker — L3.6-proyek-akhir
...
```bash
cd kelas/hari-3/L3.6-proyek-akhir/solution   # atau .../starter
docker build -t tanya-sop-l36 .
```
...
```bash
docker run --rm -p 127.0.0.1:8501:8501 tanya-sop-l36
```
...
| Ubah | Perintah |
|---|---|
| Model chat lain | `docker run -e MODEL_CHAT=qwen3:4b -p 127.0.0.1:8501:8501 tanya-sop-l36` |
| Ollama di host lain | `docker run -e OLLAMA_BASE_URL=http://192.168.1.10:11434 -p 127.0.0.1:8501:8501 tanya-sop-l36` |
...
```bash
docker run --rm -p 8501:8501 -v tanya-sop-l36-indeks:/app tanya-sop-l36
```
