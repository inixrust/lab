...
#
# PENINGKATAN 2 — kamus diperluas mengikuti korpus yang bertambah
# (peningkatan 1). Aturan yang dipakai saat memilih isinya:
#
#   - hanya singkatan yang BENAR-BENAR muncul di korpus atau di pertanyaan
#     orang. Singkatan yang tidak pernah ditanyakan hanya menambah kebisingan;
#   - kepanjangannya ditulis seperti yang tertulis di dokumen, karena inilah
#     yang harus dicocokkan BM25 kata-per-kata;
#   - istilah yang di dokumen ditulis dalam bahasa Indonesia diberi padanan
#     Indonesianya (mis. vpn -> "jaringan pribadi virtual"), bukan kepanjangan
#     Inggrisnya. Kepanjangan yang tidak ada di korpus tidak menolong siapa pun.
SINGKATAN = {
    # --- kepegawaian
    ...
    "se": "Surat Edaran",
    "sk": "Surat Keputusan",
    "sdm": "Sumber Daya Manusia kepegawaian",
    "hrd": "Divisi Sumber Daya Manusia kepegawaian",
    ...
    "po": "Purchase Order",
    "prc": "pengadaan barang dan jasa",
    "nib": "Nomor Induk Berusaha",
    "npwp": "Nomor Pokok Wajib Pajak",
    "ppn": "pajak pertambahan nilai",
    "ktr": "perjanjian kerja sama kontrak",
    "bani": "Badan Arbitrase Nasional Indonesia",
    "sla": "tingkat layanan ketersediaan",
    # --- teknologi informasi dan aset
    "ti": "Teknologi Informasi",
    "bast": "Berita Acara Serah Terima",
    "pic": "penanggung jawab",
    "2fa": "autentikasi dua faktor",
    "vpn": "jaringan pribadi virtual",
    "wfh": "kerja dari rumah kerja hibrida",
    "nr": "Notulen Rapat",
}

# Batas jumlah kepanjangan yang ditempelkan ke satu pertanyaan.
#
# Perluasan bukan barang gratis: setiap kata tambahan ikut menggeser vektor
# pertanyaan dan menambah istilah yang dihitung BM25. Pertanyaan seperti
# "Apakah SOP-05 mengatur 2FA, VPN, dan BAST?" bisa menarik lima kepanjangan
# sekaligus, dan pertanyaan aslinya tenggelam di antara istilah tempelan.
# Diambil yang muncul paling awal, karena di bahasa Indonesia pokok
# pertanyaan hampir selalu berada di depan.
MAKS_PERLUASAN = 3

# Pemecah kata untuk pencocokan singkatan. Sengaja BUKAN str.split():
# "SE-12/2026" dan "SOP-02/NCS/PRC/2026" adalah satu 'kata' bagi split(), dan
# tidak akan pernah cocok dengan kunci mana pun di kamus. Dengan pola ini,
# keduanya terurai menjadi se, 12, 2026 / sop, 02, ncs, prc, 2026 — sehingga
# pertanyaan bernomor dokumen ikut memperoleh perluasan.
POLA_KATA = re.compile(r"[a-z0-9]+")


def perluas(pertanyaan: str, maks: int = None) -> str:
    """Tambahkan kepanjangan singkatan yang muncul di pertanyaan."""
    maks = MAKS_PERLUASAN if maks is None else maks
    tambahan = []
    for kata in POLA_KATA.findall(pertanyaan.lower()):
        arti = SINGKATAN.get(kata)
        # `not in` menjaga agar "SOP-01 dan SOP-02" tidak menempelkan
        # kepanjangan yang sama dua kali.
        if arti and arti not in tambahan:
            tambahan.append(arti)
    tambahan = tambahan[:maks]
