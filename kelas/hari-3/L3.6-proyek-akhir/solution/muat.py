...
def _pemotong(jenis: str) -> RecursiveCharacterTextSplitter:
    """Peraturan, edaran, dan kontrak dipotong di batas pasal; sisanya di paragraf.

    PENINGKATAN 3 — dua hal ikut berubah di sini, dan keduanya hanya SATU baris:

      - daftar jenis berpasal pindah ke konfig.JENIS_BERPASAL, sehingga jenis
        dokumen baru (mis. 'kontrak') tidak menuntut penyuntingan berkas ini;
      - is_separator_regex diteruskan dari konfig, karena pemisahnya kini
        berupa pola regex (batas pasal, bab, DAN ayat), bukan teks tetap.
    """
    pemisah = (konfig.PEMISAH_PERATURAN if jenis in konfig.JENIS_BERPASAL
               else konfig.PEMISAH_PROSA)
    return RecursiveCharacterTextSplitter(
        chunk_size=konfig.UKURAN_POTONGAN,
        chunk_overlap=konfig.TUMPANG_TINDIH,
        separators=pemisah,
        is_separator_regex=konfig.PEMISAH_REGEX,
    )
...
def _beri_konteks(potongan, jenis: str, nama_berkas: str):
    ...
    for d in potongan:
        ...
        bagian = [str(m[k]) for k in ("source", "bab", "bagian") if m.get(k)]
        if bagian:
            d.page_content = "[" + " > ".join(bagian) + "]\n\n" + d.page_content
        hasil.append(d)
    return hasil
