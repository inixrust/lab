# Deployment on-premise dengan Docker — L3.5-graf-berkeadaan
...
```bash
cd kelas/hari-3/L3.5-graf-berkeadaan/solution   # atau .../starter
docker build -t tanya-sop-l35 .
```
...
```bash
docker run --rm -p 127.0.0.1:8501:8501 tanya-sop-l35
```
...
| Ubah | Perintah |
|---|---|
| Model chat lain | `docker run -e MODEL_CHAT=qwen3:4b -p 127.0.0.1:8501:8501 tanya-sop-l35` |
| Ollama di host lain | `docker run -e OLLAMA_BASE_URL=http://192.168.1.10:11434 -p 127.0.0.1:8501:8501 tanya-sop-l35` |
...
```bash
docker run --rm -p 8501:8501 -v tanya-sop-l35-indeks:/app tanya-sop-l35
```
